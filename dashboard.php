...
$pending_invoices_count = $pending_invoices_amount = $pending_payments = $pending_requirements = 0;
$customer_total_outstanding = 0;
$today_supplier_paid = 0;
...
if ($current_business_id) {
    ...
    // 10. Customer Total Outstanding
    $customer_outstanding_sql = "
    SELECT 
        COALESCE(
            (SELECT SUM(
                CASE 
...
";
    $stmt = $pdo->prepare($customer_outstanding_sql);
    $stmt->execute([$current_business_id, $current_business_id]);
    $customer_total_outstanding = (float) $stmt->fetchColumn();

    // 10B. Today's Supplier Paid Amount
    $today_supplier_paid_sql = "
    SELECT COALESCE(SUM(amount), 0)
    FROM payments
    WHERE DATE(payment_date) = ?
      AND type IN ('supplier', 'supplier_outstanding')
";
    $stmt = $pdo->prepare($today_supplier_paid_sql);
    $stmt->execute([$today]);
    $today_supplier_paid = (float) $stmt->fetchColumn();
...
<!doctype html>
<html lang="en">
<?php $page_title = "Dashboard - " . htmlspecialchars($current_shop_name);
            ...
            <div class="page-content">
                <div class="container-fluid">
                    ...
                    <?php if ($show_one_month_modal): ?>
                        <div class="modal fade" id="oneMonthModal" tabindex="-1" aria-labelledby="oneMonthModalLabel"
                                ...
                                <div class="modal-content border-0 shadow-lg">
                                    ...
                                    <div class="modal-body p-4">
                                        <div class="text-center mb-4">
                                            <div
                                                class="avatar-lg rounded-circle bg-warning bg-opacity-10 p-4 d-inline-block mb-3">
                                                <i class="bx bx-alarm fs-1 text-warning"></i>
                                            </div>
                                            <h4 class="mb-2">Your Cloud Subscription Expires in 30 Days!</h4>
                                            <p class="text-muted">Plan:
                                                <strong><?= htmlspecialchars($cloud_plan) ?></strong> | Expires on:
                                                <strong><?= date('d M Y', strtotime($cloud_expiry_date)) ?></strong></p>
                                            <p class="text-muted"><small>Service will expire at 11:59 PM on
                                                    <?= date('d M Y', strtotime($cloud_expiry_date)) ?></small></p>
                                        </div>
                                        ...
                                        <div class="row g-3 mb-4">
                                            ...
                                            <div class="col-md-8">
                                                <div class="card bg-light border-0 h-100">
                                                    <div class="card-body">
                                                        <h6 class="mb-3"><i
                                                                class="bx bx-info-circle me-2 text-warning"></i>Important
                                                            Information</h6>
                                                        <ul class="list-unstyled mb-0">
                                                            <li class="mb-2"><i
                                                                    class="bx bx-check-circle text-success me-2"></i> Renew
                                                                early to avoid service interruption</li>
                                                            <li class="mb-2"><i
                                                                    class="bx bx-check-circle text-success me-2"></i>
                                                                Current plan: <?= htmlspecialchars($cloud_plan) ?></li>
                                                            <li class="mb-2"><i
                                                                    class="bx bx-check-circle text-success me-2"></i> Yearly
                                                                plan recommended for best value</li>
                                                            <li class="mb-2"><i
                                                                    class="bx bx-check-circle text-success me-2"></i>
                                                                Service available until end of day on expiry date</li>
                                                            <li><i class="bx bx-check-circle text-success me-2"></i> Renew
                                                                now to get uninterrupted service</li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="text-center">
                                            ...
                                            <p class="text-muted mt-3 mb-0">
                                                <small><i class="bx bx-info-circle me-1"></i> Timer counts down to 11:59 PM
                                                    on <?= date('d M Y', strtotime($cloud_expiry_date)) ?></small>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-<?= $show_timer_in_header ? '6' : '8' ?>">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="flex-grow-1 ms-3">
                                                    <h3 class="mb-1">Welcome back, <?= htmlspecialchars($user_name) ?>!
                                                    </h3>
                                                    <p class="mb-0 opacity-75">
                                                        ...
                                                        <?= htmlspecialchars($current_shop_name) ?>
                                                        <span class="mx-2">•</span>
                                        ...
                                        <?php if ($show_timer_in_header): ?>
                                            <div class="col-md-6 mt-3 mt-md-0">
                                                <div
                                                    class="bg-white bg-opacity-10 rounded-3 p-3 border border-white border-opacity-25">
                                                    <div class="d-flex align-items-center justify-content-between">
                                                        <div class="d-flex align-items-center">
                                                            <div
                                                                class="avatar-sm rounded-circle bg-white bg-opacity-25 p-2 me-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p class="mb-0 small opacity-75">
                                                                    ...
                                                                    Date:
                                                                    <?= date('d M Y', strtotime($cloud_expiry_date)) ?>
                                                                </p>
                                                                <p class="mb-0 small opacity-75">
                                                        ...
                                                        <div class="text-end">
                                                            <div class="timer-countdown" id="headerTimer">
                                                                <div class="d-flex justify-content-end">
                                                                    <div class="timer-segment me-1">
                                                                        <span
                                                                            class="timer-value days"><?= str_pad($cloud_days_left, 2, '0', STR_PAD_LEFT) ?></span>
                                                            ...
                                                            <a href="cloud_renewal.php"
                                                                class="btn btn-sm btn-light mt-2 px-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        ...
                                        <?php if ($cloud_renewal_notification): ?>
                                            <a href="cloud_renewal.php"
                                                class="btn btn-sm btn-outline-<?= $cloud_days_left <= 3 ? 'danger' : 'warning' ?>">
                                                <i class="bx bx-cloud me-1"></i>Renew Cloud
                                                <span
                                                    class="badge bg-<?= $cloud_days_left <= 3 ? 'danger' : 'warning' ?> ms-1"><?= $cloud_days_left ?></span>
                                            </a>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                ...
                                <div class="card-body d-flex flex-column">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <div>
                                            ...
                                            <h3 class="mb-0 text-primary">₹<?= number_format($today_net_revenue, 0) ?>
                                            </h3>
                                        </div>
                                        <div class="avatar-sm">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                <span class="badge bg-primary bg-opacity-10 text-primary px-2 py-1">
                                                    <i class="bx bx-trending-up me-1"></i><?= $today_gross_sales ?>
                                                    sales
                                                </span>
                                                <?php if ($today_returns > 0): ?>
                                                    <span class="badge bg-danger bg-opacity-10 text-danger px-2 py-1 ms-1">
                                                        <i
                                                            class="bx bx-undo me-1"></i>₹<?= number_format($today_returns, 0) ?>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                            <?php if ($yesterday_net_revenue > 0): ?>
                                                ...
                                                <small class="text-<?= $class ?>">
                                                    <i
                                                        class="bx bx-<?= $growth >= 0 ? 'up-arrow-alt' : 'down-arrow-alt' ?>"></i>
                                ...
                                <div class="card-body d-flex flex-column">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <div>
                                            ...
                                            <h3 class="mb-0 text-success">₹<?= number_format($month_net_revenue, 0) ?>
                                            </h3>
                                        </div>
                                        <div class="avatar-sm">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                ...
                                                <?php if ($month_returns > 0): ?>
                                                    <span class="badge bg-danger bg-opacity-10 text-danger px-2 py-1 ms-1">
                                                        <i
                                                            class="bx bx-undo me-1"></i>₹<?= number_format($month_returns, 0) ?>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                    ...
                                    <div class="card-body d-flex flex-column">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <div>
                                                ...
                                                <h3 class="mb-0 text-warning">
                                                    ₹<?= number_format($pending_invoices_amount, 0) ?></h3>
                                            </div>
                                            <div class="avatar-sm">
                                        ...
                                        <div class="mt-auto">
                                            <span class="badge bg-warning bg-opacity-10 text-warning px-2 py-1">
                                                <i class="bx bx-receipt me-1"></i><?= $pending_invoices_count ?>
                                                invoice<?= $pending_invoices_count != 1 ? 's' : '' ?> pending
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <div>
                                                ...
                                                <h3 class="mb-0 text-danger">
                                                    ₹<?= number_format($total_supplier_outstanding, 2) ?></h3>
                                            </div>
                                            <div class="avatar-sm">
                                            ...
                                            <div class="d-flex flex-wrap gap-2 align-items-center">
                                                <?php if ($total_debit > 0): ?>
                                                    <span class="badge bg-danger bg-opacity-10 text-danger px-2 py-1">
                                                        <i class="bx bx-down-arrow-alt me-1"></i>Debit:
                                                ...
                                                <?php if ($total_credit > 0): ?>
                                                    <span class="badge bg-success bg-opacity-10 text-success px-2 py-1">
                                                        <i class="bx bx-up-arrow-alt me-1"></i>Credit:
                                                        ₹<?= number_format($total_credit, 2) ?>
                                                    </span>
                                                <?php endif; ?>

                                                <span class="badge bg-warning bg-opacity-10 text-warning px-2 py-1">
                                                    <i class="bx bx-cart me-1"></i>Pending:
                                                    ₹<?= number_format($pending_purchase_amount, 2) ?>
                                                </span>
                                            </div>
                                            <div class="mt-1 small text-muted">
                                                <i class="bx bx-info-circle me-1"></i>Net Payable = Purchase Due ±
                                                Initial Outstanding
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <div class="card-body d-flex flex-column">
                                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                                            <div>
                                                                ...
                                                                <h3 class="mb-0 text-danger">
                                                                    ₹<?= number_format($today_expenses, 0) ?></h3>
                                                            </div>
                                                            <div class="avatar-sm">
                                                                <span
                                                                    class="avatar-title bg-danger bg-opacity-10 rounded-circle fs-3">
                                                                    <i class="bx bx-money text-danger"></i>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <div class="mt-auto">
                                                            <span
                                                                class="badge bg-danger bg-opacity-10 text-danger px-2 py-1">
                                                    ...
                                                    <div class="card shadow-sm h-100 dashboard-stat-card">
                                                        <div class="card-body d-flex flex-column">
                                                            <div
                                                                ...
                                                                <div>
                                                                    <h6 class="text-muted mb-1">Customer Outstanding</h6>
                                                                    <h3 class="mb-0 text-info">
                                                                        ₹<?= number_format($customer_total_outstanding, 0) ?>
                                                                    </h3>
                                                                </div>
                                                                <div class="avatar-sm">
                                                                    <span
                                                                        class="avatar-title bg-info bg-opacity-10 rounded-circle fs-3">
                                                            ...
                                                            <div class="mt-auto">
                                                                <span
                                                                    class="badge bg-info bg-opacity-10 text-info px-2 py-1">
                                                                    <i class="bx bx-wallet me-1"></i>Total customer due
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="card shadow-sm h-100 dashboard-stat-card">
                                                        <div class="card-body d-flex flex-column">
                                                            <div
                                                                class="d-flex justify-content-between align-items-start mb-2">
                                                                <div>
                                                                    <h6 class="text-muted mb-1">Today's Supplier Paid</h6>
                                                                    <h3 class="mb-0 text-success">
                                                                        ₹<?= number_format($today_supplier_paid, 0) ?></h3>
                                                                </div>
                                                                <div class="avatar-sm">
                                                                    <span
                                                                        class="avatar-title bg-success bg-opacity-10 rounded-circle fs-3">
                                                            ...
                                                            <div class="mt-auto">
                                                                <span
                                                                    class="badge bg-success bg-opacity-10 text-success px-2 py-1 me-1">
                                                                    <i class="bx bx-check-circle me-1"></i>Paid today
                                                                </span>
                                                                <span
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-6">
                                                                <div class="d-flex justify-content-between align-items-center">
                                                                    <span>Today:</span>
                                                                    <span
                                                                        class="fw-bold">₹<?= number_format($today_returns, 0) ?>
                                                                        (<?= number_format($today_return_percentage, 1) ?>%)</span>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <div class="d-flex justify-content-between align-items-center">
                                                                    ...
                                                                    <span
                                                                        class="fw-bold">₹<?= number_format($month_returns, 0) ?>
                                                                        (<?= number_format($month_return_percentage, 1) ?>%)</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="card bg-light border-0 h-100">
                                                    <div class="card-body text-center">
                                                        <h2 class="text-primary mb-1">
                                                            ₹<?= number_format($shop_stock_value, 0) ?></h2>
                                            ...
                                            <div class="col-xl-3 col-md-6">
                                                <a href="products.php?stock=low" class="text-decoration-none">
                                                    <div class="card bg-light border-0 h-100">
                                                        <div class="card-body text-center">
                                                            <h2
                                                                class="<?= $low_stock_items > 0 ? 'text-warning' : 'text-success' ?> mb-1">
                                                                <?= $low_stock_items ?></h2>
                                            ...
                                            <div class="col-xl-3 col-md-6">
                                                <a href="products.php?stock=out" class="text-decoration-none">
                                                    <div class="card bg-light border-0 h-100">
                                                        <div class="card-body text-center">
                                                            <h2
                                                                class="<?= $out_of_stock > 0 ? 'text-danger' : 'text-success' ?> mb-1">
                                                                <?= $out_of_stock ?></h2>
                                        ...
                                        <?php if (!empty($recent_sales)): ?>
                                            <?php foreach ($recent_sales as $sale):
                                                ...
                                                <div class="list-group-item border-0 px-0 py-2">
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        <div>
                                                            <h6 class="mb-0"><?= htmlspecialchars($sale['invoice_number']) ?>
                                                            </h6>
                                                            <small
                                                                class="text-muted"><?= htmlspecialchars($sale['customer']) ?></small>
                                                            <?php if ($has_returns): ?>
                                                                <div>
                                                                    <small class="text-danger">
                                                                        <i class="bx bx-undo me-1"></i> Returns:
                                                                        ₹<?= number_format($sale['returns'], 0) ?>
                                                                    </small>
                                                                </div>
                                                            <?php endif; ?>
                                                        </div>
                                                        <div class="text-end">
                                                            ...
                                                            <?php if ($has_returns): ?>
                                                                <div>
                                                                    <small class="text-muted">
                                                                        <s
                                                                            class="text-danger">₹<?= number_format($sale['gross_total'], 0) ?></s>
                                                                    </small>
                                                                </div>
                                                            <?php endif; ?>
                                                            <div>
                                                                <small
                                                                    class="text-muted"><?= date('h:i A', strtotime($sale['created_at'])) ?></small>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div class="card shadow-sm">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        ...
                                        <div class="d-flex align-items-center gap-2 bg-light rounded-pill px-3 py-2">
                                            <span
                                                class="badge <?= $cloud_renewal_notification ? ($cloud_days_left <= 3 ? 'bg-danger' : 'bg-warning') : 'bg-success' ?> rounded-circle p-1">
