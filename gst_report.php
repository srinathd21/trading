...
$shopsStmt = $pdo->prepare("
    ...
    ORDER BY shop_name
");
$shopsStmt->execute([$current_business_id]);
$shops = $shopsStmt->fetchAll(PDO::FETCH_ASSOC);

function getShopName($shops, $shop_id)
{
    ...
    foreach ($shops as $shop) {
        if ((string)$shop['id'] === (string)$shop_id) {
...
function getGSTSummary($pdo, $business_id, $start_date, $end_date, $shop_id)
{
    ...
    $query = "
        SELECT 
            ...
            SUM(COALESCE(ii.taxable_value, 0)) as total_taxable_value,
            SUM(COALESCE(ii.cgst_amount, 0)) as total_cgst,
            SUM(COALESCE(ii.sgst_amount, 0)) as total_sgst,
            SUM(COALESCE(ii.igst_amount, 0)) as total_igst,
            SUM(COALESCE(ii.cgst_amount, 0) + COALESCE(ii.sgst_amount, 0) + COALESCE(ii.igst_amount, 0)) as total_gst,
            SUM(COALESCE(ii.total_with_gst, 0)) as invoice_total
        FROM invoices i
        ...
        WHERE i.business_id = ?
          ...
          AND i.gst_status = 1
          $shop_condition
        GROUP BY i.id, DATE(i.created_at), i.invoice_number, i.customer_type, i.gst_status, s.shop_name, c.name, c.gstin
...
function getHSNWiseGST($pdo, $business_id, $start_date, $end_date, $shop_id)
{
    ...
    $query = "
        SELECT 
            COALESCE(ii.hsn_code, 'NO-HSN') as hsn_code,
            COALESCE(p.product_name, ii.product_name, 'Unknown Product') as product_name,
            ...
            SUM(COALESCE(ii.sgst_amount, 0)) as total_sgst,
            SUM(COALESCE(ii.igst_amount, 0)) as total_igst,
            ROUND(AVG(COALESCE(ii.cgst_rate, 0)), 2) as avg_cgst_rate,
            ROUND(AVG(COALESCE(ii.sgst_rate, 0)), 2) as avg_sgst_rate,
            ROUND(AVG(COALESCE(ii.igst_rate, 0)), 2) as avg_igst_rate
        ...
        WHERE i.business_id = ?
          ...
          AND i.gst_status = 1
          $shop_condition
        GROUP BY COALESCE(ii.hsn_code, 'NO-HSN'), COALESCE(p.product_name, ii.product_name, 'Unknown Product')
...
function getMonthlyGSTTrend($pdo, $business_id, $year, $shop_id)
{
    ...
    $query = "
        ...
        WHERE i.business_id = ?
          ...
          AND i.gst_status = 1
          $shop_condition
        GROUP BY DATE_FORMAT(i.created_at, '%Y-%m'), DATE_FORMAT(i.created_at, '%M %Y')
        ORDER BY month
    ";
...
// HSN-wise amount per invoice for export
function getInvoiceHSNSummary($pdo, $invoice_id)
{
    $stmt = $pdo->prepare("
        SELECT 
            COALESCE(ii.hsn_code, 'NO-HSN') as hsn_code,
            SUM(COALESCE(ii.taxable_value, 0)) as hsn_taxable_value
        FROM invoice_items ii
        WHERE ii.invoice_id = ?
        GROUP BY COALESCE(ii.hsn_code, 'NO-HSN')
        ORDER BY hsn_code
    ");
    ...
    $parts = [];
    foreach ($rows as $row) {
        $parts[] = $row['hsn_code'] . ': ' . number_format((float)$row['hsn_taxable_value'], 2, '.', '');
    }
...
if ($report_type === 'summary') {
    foreach ($gst_summary as $row) {
        $total_taxable += (float)$row['total_taxable_value'];
        ...
        $total_invoice_amount += (float)$row['invoice_total'];
        $total_invoices++;
    }
}
...
$gstRatesStmt = $pdo->prepare("
    SELECT hsn_code, description, cgst_rate, sgst_rate, igst_rate, status 
    FROM gst_rates 
    WHERE business_id = ? 
    ORDER BY hsn_code
");
$gstRatesStmt->execute([$current_business_id]);
$gst_rates_list = $gstRatesStmt->fetchAll(PDO::FETCH_ASSOC);
...
function exportToCSV($report_type, $start_date, $end_date, $shop_id)
{
    ...
    $shop_name = getShopName($shops, $shop_id);
    ...
    if ($report_type === 'summary') {
        ...
        fputcsv($output, ['Date', 'Invoice No', 'Customer', 'Customer GSTIN', 'Customer Type', 'Shop', 'HSN Wise Amount', 'Taxable Value', 'CGST', 'SGST', 'IGST', 'Total GST', 'Invoice Total']);
        ...
        foreach ($data as $row) {
            $hsn_wise_amount = getInvoiceHSNSummary($pdo, $row['invoice_id']);
            ...
            fputcsv($output, [
                ...
                $row['shop_name'],
                $hsn_wise_amount,
                number_format((float)$row['total_taxable_value'], 2, '.', ''),
                number_format((float)$row['total_cgst'], 2, '.', ''),
                number_format((float)$row['total_sgst'], 2, '.', ''),
                number_format((float)$row['total_igst'], 2, '.', ''),
                number_format((float)$row['total_gst'], 2, '.', ''),
                number_format((float)$row['invoice_total'], 2, '.', '')
            ]);

            $grand_taxable += (float)$row['total_taxable_value'];
            $grand_cgst += (float)$row['total_cgst'];
            $grand_sgst += (float)$row['total_sgst'];
            $grand_igst += (float)$row['total_igst'];
            $grand_gst += (float)$row['total_gst'];
            $grand_total += (float)$row['invoice_total'];
        }
        ...
        foreach ($data as $row) {
            fputcsv($output, [
                ...
                $row['invoice_count'],
                $row['total_quantity'],
                number_format((float)$row['avg_cgst_rate'], 2),
                number_format((float)$row['avg_sgst_rate'], 2),
                number_format((float)$row['avg_igst_rate'], 2),
                number_format((float)$row['total_taxable_value'], 2, '.', ''),
                number_format((float)$row['total_cgst'], 2, '.', ''),
                number_format((float)$row['total_sgst'], 2, '.', ''),
                number_format((float)$row['total_igst'], 2, '.', ''),
                number_format((float)$row['total_cgst'] + (float)$row['total_sgst'] + (float)$row['total_igst'], 2, '.', '')
            ]);
        ...
        foreach ($data as $row) {
            $gst_percentage = (float)$row['total_taxable_value'] > 0
                ? (((float)$row['total_gst']) / ((float)$row['total_taxable_value'])) * 100
                : 0;
            ...
            $yearly_taxable += (float)$row['total_taxable_value'];
            $yearly_cgst += (float)$row['total_cgst'];
            $yearly_sgst += (float)$row['total_sgst'];
            $yearly_igst += (float)$row['total_igst'];
            $yearly_gst += (float)$row['total_gst'];
        }
...
function exportToExcel($report_type, $start_date, $end_date, $shop_id)
{
    ...
    echo "<tr><td colspan='13' class='header' style='text-align:center;'>";
    ...
    echo "<p>Shop: " . htmlspecialchars($shop_name) . "</p>";
    ...
    if ($report_type === 'summary') {
        ...
        echo "<th>Customer</th>";
        echo "<th>Customer GSTIN</th>";
        echo "<th>Customer Type</th>";
        echo "<th>Shop</th>";
        echo "<th>HSN Wise Amount</th>";
...
echo "<td>" . htmlspecialchars((string)($row['customer_gstin'] ?? ''), ENT_QUOTES, 'UTF-8') . "</td>";
echo "<td>" . htmlspecialchars((string)ucfirst((string)($row['customer_type'] ?? '')), ENT_QUOTES, 'UTF-8') . "</td>";
echo "<td>" . htmlspecialchars((string)($row['shop_name'] ?? ''), ENT_QUOTES, 'UTF-8') . "</td>";
echo "<td>" . htmlspecialchars((string)($hsn_wise_amount ?? ''), ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td>" . number_format((float)$row['total_taxable_value'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_cgst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_sgst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_igst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_gst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['invoice_total'], 2) . "</td>";
            echo "</tr>";
            ...
            $total_igst += (float)$row['total_igst'];
            $total_gst += (float)$row['total_gst'];
            $total_invoice += (float)$row['invoice_total'];
        }
        ...
        foreach ($data as $row) {
            ...
            echo "<td>" . number_format((float)$row['total_quantity'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['avg_cgst_rate'], 2) . "%</td>";
            echo "<td>" . number_format((float)$row['avg_sgst_rate'], 2) . "%</td>";
            echo "<td>" . number_format((float)$row['avg_igst_rate'], 2) . "%</td>";
            echo "<td>" . number_format((float)$row['total_taxable_value'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_cgst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_sgst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_igst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_cgst'] + (float)$row['total_sgst'] + (float)$row['total_igst'], 2) . "</td>";
            echo "</tr>";
        ...
        foreach ($data as $row) {
            ...
            echo "<tr>";
            echo "<td>" . htmlspecialchars($row['month_name']) . "</td>";
            echo "<td>" . (int)$row['invoice_count'] . "</td>";
            echo "<td>" . number_format((float)$row['total_taxable_value'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_cgst'], 2) . "</td>";
            echo "<td>" . number_format((float)$row['total_sgst'], 2) . "</td>";
...
function exportToJSON($report_type, $start_date, $end_date, $shop_id)
{
    ...
    if ($report_type === 'summary') {
        $data = getGSTSummary($pdo, $current_business_id, $start_date, $end_date, $shop_id);
        foreach ($data as &$row) {
            $row['hsn_wise_amount'] = getInvoiceHSNSummary($pdo, $row['invoice_id']);
        }
        unset($row);
        ...
        $report_data['summary'] = [
            ...
            'total_gst' => array_sum(array_map(function ($item) {
                return (float)$item['total_cgst'] + (float)$item['total_sgst'] + (float)$item['total_igst'];
            }, $data))
        ];
    } elseif ($report_type === 'monthly') {
        $year = date('Y');
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="shop_id" class="form-select">
                                            ...
                                            <?php foreach ($shops as $shop): ?>
                                                <option value="<?= $shop['id'] ?>" <?= (string)$shop_id === (string)$shop['id'] ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($shop['shop_name']) ?> (<?= htmlspecialchars($shop['shop_code']) ?>)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                            ...
                            <div class="card-header bg-light">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="text-muted">
                                        ...
                                        <?php if ($shop_id !== 'all'): ?>
                                            | Shop: <?= htmlspecialchars(getShopName($shops, $shop_id)) ?>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($gst_summary as $invoice): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="invoice_view.php?id=<?= (int)$invoice['invoice_id'] ?>" class="text-decoration-none">
                                                        ...
                                                        <td>
                                                            ...
                                                            <small class="text-muted">
                                                                <?= htmlspecialchars($invoice['customer_type']) ?>
                                                        ...
                                                        <td class="text-end">₹<?= number_format((float)$invoice['total_taxable_value'], 2) ?></td>
                                                        <td class="text-end">
                                                            <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-1">
                                                                ₹<?= number_format((float)$invoice['total_cgst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <span class="badge bg-success bg-opacity-10 text-success px-3 py-1">
                                                                ₹<?= number_format((float)$invoice['total_sgst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <span class="badge bg-warning bg-opacity-10 text-warning px-3 py-1">
                                                                ₹<?= number_format((float)$invoice['total_igst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <strong class="text-dark">₹<?= number_format((float)$invoice['total_gst'], 2) ?></strong>
                                                        </td>
                                                        <td class="text-end">
                                                            <strong>₹<?= number_format((float)$invoice['invoice_total'], 2) ?></strong>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                                    ...
                                                    <div class="composition-body">
                                                        <div class="distribution-item">
                                                            ...
                                                            <div class="distribution-progress">
                                                                <div class="progress-bar bg-primary" role="progressbar" style="width: <?= $total_gst > 0 ? ($total_cgst / $total_gst) * 100 : 0 ?>%;"></div>
                                                            </div>
                                                        ...
                                                        <div class="distribution-item">
                                                            ...
                                                            <div class="distribution-progress">
                                                                <div class="progress-bar bg-success" role="progressbar" style="width: <?= $total_gst > 0 ? ($total_sgst / $total_gst) * 100 : 0 ?>%;"></div>
                                                            </div>
                                                        ...
                                                        <div class="distribution-item">
                                                            ...
                                                            <div class="distribution-progress">
                                                                <div class="progress-bar bg-warning" role="progressbar" style="width: <?= $total_gst > 0 ? ($total_igst / $total_gst) * 100 : 0 ?>%;"></div>
                                                            </div>
                                                        ...
                                                        <div class="row mt-4">
                                                            <div class="col-6">
                                                                <div class="p-3 border rounded text-center">
                                                                    <small class="text-muted d-block">Taxable Value</small>
                                                                    <h6 class="mb-0"><?= $total_invoice_amount > 0 ? number_format(($total_taxable / $total_invoice_amount) * 100, 1) : 0 ?>%</h6>
                                                            ...
                                                            <div class="col-6">
                                                                <div class="p-3 border rounded text-center">
                                                                    <small class="text-muted d-block">Total GST</small>
                                                                    <h6 class="mb-0 text-purple"><?= $total_invoice_amount > 0 ? number_format(($total_gst / $total_invoice_amount) * 100, 1) : 0 ?>%</h6>
                                                                    <small class="text-muted">of Grand Total</small>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <tr class="hsn-row">
                                                        <td>
                                                            ...
                                                            foreach ($gst_rates_list as $rate) {
                                                                if ((string)$rate['hsn_code'] === (string)$hsn['hsn_code']) {
                                                                    $gst_rate = $rate;
                                                                    break;
                                                                }
                                                            }
                                                            ...
                                                            <?php if ($gst_rate): ?>
                                                                <br>
                                                                <small class="text-muted">
                                                                    <?= htmlspecialchars($gst_rate['description']) ?>
                                                                    <span class="badge bg-<?= $gst_rate['status'] === 'active' ? 'success' : 'secondary' ?> ms-1">
                                                                        <?= htmlspecialchars($gst_rate['status']) ?>
                                                                    </span>
                                                                </small>
                                                            <?php endif; ?>
                                                        </td>
                                                        ...
                                                        <td class="text-center">
                                                            <span class="badge bg-info rounded-pill px-3">
                                                                <?= (int)$hsn['invoice_count'] ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-center">
                                                            <span class="badge bg-secondary bg-opacity-10 text-secondary px-3">
                                                                <?= number_format((float)$hsn['total_quantity']) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-center">
                                                            <?= (float)$hsn['avg_cgst_rate'] ? number_format((float)$hsn['avg_cgst_rate'], 2) . '%' : '-' ?>
                                                        </td>
                                                        <td class="text-center">
                                                            <?= (float)$hsn['avg_sgst_rate'] ? number_format((float)$hsn['avg_sgst_rate'], 2) . '%' : '-' ?>
                                                        </td>
                                                        <td class="text-center">
                                                            <?= (float)$hsn['avg_igst_rate'] ? number_format((float)$hsn['avg_igst_rate'], 2) . '%' : '-' ?>
                                                        </td>
                                                        <td class="text-end">₹<?= number_format((float)$hsn['total_taxable_value'], 2) ?></td>
                                                        <td class="text-end">
                                                            <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-1">
                                                                ₹<?= number_format((float)$hsn['total_cgst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <span class="badge bg-success bg-opacity-10 text-success px-3 py-1">
                                                                ₹<?= number_format((float)$hsn['total_sgst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <span class="badge bg-warning bg-opacity-10 text-warning px-3 py-1">
                                                                ₹<?= number_format((float)$hsn['total_igst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <strong>₹<?= number_format((float)$hsn['total_cgst'] + (float)$hsn['total_sgst'] + (float)$hsn['total_igst'], 2) ?></strong>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                        ...
                                        <table class="table table-hover" id="monthlyTable">
                                            ...
                                            <?php else: ?>
                                                ...
                                                <?php foreach ($monthly_trend as $month): ?>
                                                    <?php
                                                    $yearly_total_taxable += (float)$month['total_taxable_value'];
                                                    $yearly_total_cgst += (float)$month['total_cgst'];
                                                    $yearly_total_sgst += (float)$month['total_sgst'];
                                                    $yearly_total_igst += (float)$month['total_igst'];
                                                    $yearly_total_gst += (float)$month['total_gst'];
                                                    $gst_percentage = (float)$month['total_taxable_value'] > 0
                                                        ? (((float)$month['total_gst']) / ((float)$month['total_taxable_value'])) * 100
                                                        : 0;
                                                    ?>
                                                    <tr>
                                                        <td>
                                                        ...
                                                        <td class="text-center">
                                                            <span class="badge bg-info rounded-pill px-3">
                                                                <?= (int)$month['invoice_count'] ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">₹<?= number_format((float)$month['total_taxable_value'], 2) ?></td>
                                                        <td class="text-end">
                                                            <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-1">
                                                                ₹<?= number_format((float)$month['total_cgst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <span class="badge bg-success bg-opacity-10 text-success px-3 py-1">
                                                                ₹<?= number_format((float)$month['total_sgst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <span class="badge bg-warning bg-opacity-10 text-warning px-3 py-1">
                                                                ₹<?= number_format((float)$month['total_igst'], 2) ?>
                                                            </span>
                                                        </td>
                                                        <td class="text-end">
                                                            <strong>₹<?= number_format((float)$month['total_gst'], 2) ?></strong>
                                                        </td>
                                                        <td class="text-end">
                                                ...
                                                <?php $yearly_gst_percentage = $yearly_total_taxable > 0 ? ($yearly_total_gst / $yearly_total_taxable) * 100 : 0; ?>
                                                <tr class="table-total-row">
...
$(document).ready(function() {
    ...
    const monthlyData = {
        ...
        datasets: [
            {
                label: 'CGST',
                data: <?= json_encode(array_map('floatval', array_column($monthly_trend, 'total_cgst'))) ?>,
            ...
            {
                label: 'SGST',
                data: <?= json_encode(array_map('floatval', array_column($monthly_trend, 'total_sgst'))) ?>,
            ...
            {
                label: 'IGST',
                data: <?= json_encode(array_map('floatval', array_column($monthly_trend, 'total_igst'))) ?>,
            ...
            {
                label: 'Total GST',
                data: <?= json_encode(array_map('floatval', array_column($monthly_trend, 'total_gst'))) ?>,
    ...
    $('.export-link').click(function(e) {
        ...
        params.append('year', '<?= htmlspecialchars($year) ?>');
        <?php endif; ?>
