...
document.getElementById('date').value = now.toISOString().slice(0, 16);
    ...
    async function checkAndGenerateInvoiceNumber(force = false) {
        try {
            ...
            const response = await fetchWithTimeout('api/paint_invoices.php?action=get_next_invoice_number', {
                method: 'POST',
                headers: {
...
if (invoiceDateInput) {
    invoiceDateInput.addEventListener('change', function () {
        ...
        try {
            ...
            if (barcodeInput && !document.getElementById('manual-product-name')) {
...
document.getElementById('invoice-type').addEventListener('change', function () {
    ...
    function handleBarcodeScan(event) {
        if (document.getElementById('manual-product-name')) {
        ...
        try {
            ...
            if (addBtn) {
                if (document.getElementById('manual-product-name')) {
                    ...
                    addBtn.title = 'Add manual paint item';
                } else {
...
document.getElementById('invoice-type').addEventListener('change', function () {
    ...
    function updateCartItemQuantity(index, newQty) {
        try {
            ...
            if (!product) {
                // Manual seller sale: product table check removed
            }
...
`;
    ...
    function buildInvoiceItemsForPayload(totals) {
        ...
        return CART.map((item, index) => {
            ...
            return {
                product_id: (item.is_manual_item || item.manual_item) ? 0 : (item.product_id || 0),
                is_manual_item: (item.is_manual_item || item.manual_item) ? 1 : 0,
                manual_item: (item.is_manual_item || item.manual_item) ? 1 : 0,
                name: item.name || item.product_name || '',
                product_name: item.name || item.product_name || '',
                product_name_snapshot: item.name || item.product_name || '',
                code: item.code || '',
                quantity: item.quantity,
                unit: item.unit || 'pcs',
                price: item.price,
                price_type: (item.is_manual_item || item.manual_item) ? 'manual' : (item.price_type || 'retail'),
    ...
    async function generateBill() {
        try {
            ...
            try {
                ...
                const response = await fetchWithTimeout('api/paint_invoices.php?action=save', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
...
created_at: formatDateTimeForMysql(document.getElementById('date').value),
            ...
            try {
                const response = await fetchWithTimeout('api/paint_invoices.php?action=save_for_print', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="d-flex justify-content-between mb-2">
                                                <span class="fw-bold">Total Selling Value</span>
...
`;
    ...
    /* =========================================================
       PAINT SALE MANUAL ENTRY OVERRIDES
       Seller enters: product name, quantity, HSN/GST, selling price.
       No product master selection and no stock deduction from frontend.
    ========================================================= */
    function populatePaintHsnDropdown() {
        const hsnSelect = document.getElementById('manual-hsn-select');
        if (!hsnSelect) return;

        const currentValue = hsnSelect.value || '';
        hsnSelect.innerHTML = '<option value="">Loading HSN...</option>';

        fetchWithTimeout('api/gst_rates.php?action=list', {
            timeout: 8000,
            retries: 1,
            credentials: 'include',
            ...
            .then(data => {
                hsnSelect.innerHTML = '<option value="">-- Select HSN --</option>';

                if (!data.success || !Array.isArray(data.rates) || data.rates.length === 0) {
                    const opt = document.createElement('option');
                    opt.value = '';
                    opt.textContent = 'No GST rates found for this business';
                    hsnSelect.appendChild(opt);

                    const nonGstOpt = document.createElement('option');
                    nonGstOpt.value = 'NON-GST';
                    nonGstOpt.dataset.cgst = 0;
                    nonGstOpt.dataset.sgst = 0;
                    nonGstOpt.dataset.igst = 0;
                    nonGstOpt.textContent = 'NON-GST - 0%';
                    hsnSelect.appendChild(nonGstOpt);

                    if (typeof showToast === 'function') {
                        showToast(data.message || 'No active GST rates found for this business', 'warning');
                    }
                    return;
                }
                ...
                data.rates.forEach(row => {
                    ...
                    const cgst = parseFloat(row.cgst_rate) || 0;
                    const sgst = parseFloat(row.sgst_rate) || 0;
                    const igst = parseFloat(row.igst_rate) || 0;
                    const total = cgst + sgst + igst;
                    const desc = String(row.description || '').trim();

                    const opt = document.createElement('option');
                    opt.value = hsn;
                    opt.dataset.cgst = cgst;
                    opt.dataset.sgst = sgst;
                    opt.dataset.igst = igst;
                    opt.dataset.description = desc;
                    opt.textContent = `${hsn}${desc ? ' - ' + desc : ''} - GST ${total.toFixed(2)}%`;
                    hsnSelect.appendChild(opt);
                });
                ...
                if (currentValue) {
                    hsnSelect.value = currentValue;
                }
            })
            .catch(error => {
                console.error('GST rates load failed:', error);
                hsnSelect.innerHTML = '<option value="">Failed to load HSN</option>';
                ...
                if (typeof showToast === 'function') {
                    showToast('Failed to load GST rates from logged-in business', 'danger');
                }
            });
    }
    function getSelectedManualHsn() {
        const hsnSelect = document.getElementById('manual-hsn-select');
        if (!hsnSelect || !hsnSelect.value) {
            return { hsn: '', cgst: 0, sgst: 0, igst: 0 };
        }

        const opt = hsnSelect.options[hsnSelect.selectedIndex];
        const hsn = String(hsnSelect.value || '').trim();

        if (hsn === 'NON-GST') {
            return { hsn: 'NON-GST', cgst: 0, sgst: 0, igst: 0 };
        }

        return {
            hsn: hsn,
            cgst: parseFloat(opt?.dataset?.cgst || 0) || 0,
            sgst: parseFloat(opt?.dataset?.sgst || 0) || 0,
            igst: parseFloat(opt?.dataset?.igst || 0) || 0
        };
    }

    function resetPaintManualInputs() {
    ...
    function addProductToCart() {
        try {
            ...
            const sellingPrice = parseFloat(priceInput?.value || '0') || 0;
            const hsnData = getSelectedManualHsn();

            if (!productName) {
                showToast('Please enter product / paint description', 'warning');
                if (nameInput) nameInput.focus();
                return;
            }
            ...
            if (sellingPrice <= 0) {
                showToast('Please enter valid selling price', 'warning');
                if (priceInput) priceInput.focus();
                return;
            }

            if (!hsnData.hsn) {
                showToast('Please select HSN / GST', 'warning');
                const hsnSelect = document.getElementById('manual-hsn-select');
                if (hsnSelect) hsnSelect.focus();
                return;
            }
            ...
            const manualItem = {
                ...
                discount_type: 'percentage',
                discount_amount: 0,
                hsn_code: hsnData.hsn === 'NON-GST' ? '' : hsnData.hsn,
                cgst_rate: hsnData.cgst,
                sgst_rate: hsnData.sgst,
                igst_rate: hsnData.igst,
                stock_price: 0,
                referral_enabled: 0,
                referral_type: '',
                referral_value: 0,
            ...
            updateBillingSummary();
            updateButtonStates();
            resetPaintManualInputs();
            showToast('Paint item added to cart', 'success');

        } catch (error) {
            console.error('Manual paint add error:', error);
            showToast('Unable to add paint item', 'danger');
        }
    }
    ...
    function renderCart() {
        try {
            ...
            if (!Array.isArray(CART) || CART.length === 0) {
                tbody.innerHTML = '<tr id="emptyCartRow"><td colspan="10" class="cart-empty">No items in cart</td></tr>';
                return;
            }
            ...
            CART.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="text-start">
                        <strong>${escapeHtml(item.name || '')}</strong><br>
                        <small class="text-muted">HSN: ${escapeHtml(item.hsn_code || 'NON-GST')}</small>
                        ${item.is_manual_item ? '<br><span class="manual-item-badge">MANUAL PAINT SALE</span>' : ''}
                    </td>
                    <td>
                        <div class="d-flex align-items-center justify-content-center gap-1">
                            <button class="btn btn-sm btn-outline-secondary py-0 px-2" type="button" onclick="cartItemDecrement(${index})">
                                <i class="fas fa-minus"></i>
                            </button>
                            <input type="number" class="form-control form-control-sm text-center cart-qty-input"
                                   value="${parseFloat(item.quantity || 0)}" data-index="${index}" min="0.001" step="0.001" style="width:80px;">
                            <button class="btn btn-sm btn-outline-secondary py-0 px-2" type="button" onclick="cartItemIncrement(${index})">
                                <i class="fas fa-plus"></i>
                            </button>
                        </div>
                    </td>
                    <td>${escapeHtml(item.unit || 'pcs')}</td>
                    <td><span class="price-type-badge retail-badge">Manual</span></td>
                    <td>
                        <div class="d-flex align-items-center gap-1">
                            <input type="number" class="form-control form-control-sm text-center cart-discount-value"
                                   value="${item.discount_value || 0}" data-index="${index}" min="0" step="0.01" style="width:70px;">
                            <select class="form-select form-select-sm cart-discount-type" data-index="${index}" style="width:70px;">
                                <option value="percentage" ${item.discount_type === 'percentage' ? 'selected' : ''}>%</option>
                                <option value="fixed" ${item.discount_type === 'fixed' ? 'selected' : ''}>₹</option>
                            </select>
                    ...
                    <td class="text-end">₹${(parseFloat(item.price) || 0).toFixed(2)}<br><small class="text-muted">Per ${escapeHtml(item.unit || 'pcs')}</small></td>
                    <td class="text-end">${gstRate.toFixed(2)}%</td>
                    <td class="text-end">₹${itemTotal.toFixed(2)}</td>
                    <td>
                        <button class="btn btn-sm btn-outline-danger" type="button" onclick="removeCartItem(${index})" title="Remove item">
                            <i class="fas fa-trash"></i>
                        </button>
                    </td>`;
                tbody.appendChild(row);
            ...
            document.querySelectorAll('.cart-qty-input').forEach(input => {
                input.addEventListener('change', function () {
                    updateCartItemQuantity(parseInt(this.dataset.index, 10), parseFloat(this.value) || 1);
                });
            });
            document.querySelectorAll('.cart-discount-value').forEach(input => {
                input.addEventListener('change', function () {
                    updateCartItemDiscount(parseInt(this.dataset.index, 10), parseFloat(this.value) || 0);
                });
            });
            document.querySelectorAll('.cart-discount-type').forEach(select => {
                select.addEventListener('change', function () {
                    updateCartItemDiscountType(parseInt(this.dataset.index, 10), this.value);
                });
            });

        } catch (error) {
            console.error('Paint render cart error:', error);
            showToast('Error displaying cart', 'danger');
        }
    }
    ...
    function cartItemDecrement(index) {
        if (!CART[index]) return;
        const newQty = Math.max(0.001, (parseFloat(CART[index].quantity) || 1) - 1);
        updateCartItemQuantity(index, newQty);
    }

    function cartItemIncrement(index) {
        if (!CART[index]) return;
        updateCartItemQuantity(index, (parseFloat(CART[index].quantity) || 1) + 1);
    }

    function updateCartItemDiscount(index, discountValue) {
        if (!CART[index]) return;
        const discountTypeElement = document.querySelector(`.cart-discount-type[data-index="${index}"]`);
        const discountType = discountTypeElement ? discountTypeElement.value : (CART[index].discount_type || 'percentage');
        discountValue = parseFloat(discountValue) || 0;
        if (discountType === 'percentage' && discountValue > 100) discountValue = 100;
        CART[index].discount_value = Math.max(0, discountValue);
    ...
    function updateCartItemDiscountType(index, discountType) {
        if (!CART[index]) return;
        CART[index].discount_type = discountType || 'percentage';
        saveCartToSession();
        updateBillingSummary();
        renderCart();
    }
    ...
    function updateCartItemPriceType(index, priceType) {
        // Paint sale uses manual selling price. No product price type change needed.
        if (!CART[index]) return;
        CART[index].price_type = 'manual';
        saveCartToSession();
        updateBillingSummary();
        renderCart();
    }

    // Rebuild HSN dropdown after product API loads.
    const originalLoadProductsForPaint = window.loadProducts;
    if (typeof originalLoadProductsForPaint === 'function') {
        window.loadProducts = async function () {
            const result = await originalLoadProductsForPaint.apply(this, arguments);
            populatePaintHsnDropdown();
            return result;
        };
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        ['manual-product-name', 'qty-input', 'manual-hsn-select', 'selling-price'].forEach(id => {
            ...
            el.addEventListener('keydown', function (e) {
                if (e.key === 'Enter') {
                    ...
                    if (id === 'selling-price') {
                        ...
                        const order = ['manual-product-name', 'qty-input', 'manual-hsn-select', 'selling-price'];
                        const next = document.getElementById(order[order.indexOf(id) + 1]);
                        if (next) next.focus();
                    }
                }
            });
...
/* =========================================================
   SELLER MANUAL SALE - NO PRODUCTS TABLE CHECK
   This final override removes all product lookup/stock logic.
   Seller entered products are invoice-only items.
========================================================= */
(function () {
    function isManualSellerSale() {
        return !!document.getElementById('manual-product-name');
    }
    ...
    function hsnData() {
        const hsnSelect = document.getElementById('manual-hsn-select');
        if (!hsnSelect) return { hsn: '', cgst: 0, sgst: 0, igst: 0 };

        const opt = hsnSelect.options[hsnSelect.selectedIndex];
        const rawHsn = hsnSelect.value || opt?.dataset?.hsn || '';
        const nonGst = String(rawHsn).toUpperCase() === 'NON-GST';

        return {
            hsn: nonGst ? '' : rawHsn,
            cgst: nonGst ? 0 : num(opt?.dataset?.cgst || opt?.getAttribute('data-cgst') || 0),
            sgst: nonGst ? 0 : num(opt?.dataset?.sgst || opt?.getAttribute('data-sgst') || 0),
            igst: nonGst ? 0 : num(opt?.dataset?.igst || opt?.getAttribute('data-igst') || 0)
        };
    }

    // No product lookup needed for manual seller sale.
    window.findProductById = function () { return null; };
    window.findProductByBarcode = function () { return null; };
    ...
    window.addProductToCart = function () {
        try {
            ...
            if (!nameInput) {
                showToast('Manual product field missing', 'danger');
                return;
            }
            ...
            if (price <= 0) {
                showToast('Enter selling price', 'warning');
                priceInput?.focus();
                return;
            }

            CART.push({
                cart_id: 'manual_' + Date.now() + '_' + Math.floor(Math.random() * 9999),
                product_id: 0,
                is_manual_item: true,
                manual_item: true,
                no_stock_reduce: true,
            ...
            if (typeof saveCartToSession === 'function') saveCartToSession();
            ...
            const hsnSelect = document.getElementById('manual-hsn-select');
            if (hsnSelect) hsnSelect.selectedIndex = 0;
            ...
            nameInput.focus();
            showToast('Manual sale item added', 'success');
        } catch (error) {
            console.error('Manual seller sale add error:', error);
            showToast('Unable to add manual item', 'danger');
        }
    };

    window.buildInvoiceItemsForPayload = function (totals) {
        const distribution = totals?.discountDistribution || CART.map(() => 0);

        return CART.map(function (item, index) {
            const qty = num(item.quantity);
            const price = num(item.price);
            const discountShare = num(distribution[index] || item.overall_discount_share || item.discount_amount || 0);
            const lineBefore = qty * price;
            const lineAfter = Math.max(0, lineBefore - discountShare);

            let gst = { taxable: lineAfter, cgst: 0, sgst: 0, igst: 0 };
            if (typeof calculateItemGST === 'function') {
                gst = calculateItemGST(item, discountShare);
            }
            ...
            return {
                ...
                discount_value: discountShare,
                discount_type: 'fixed',
                discount_amount: discountShare,
                overall_discount_share: discountShare,

                total: parseFloat(lineAfter.toFixed(2)),

                hsn_code: item.hsn_code || '',
                cgst_rate: num(item.cgst_rate),
                sgst_rate: num(item.sgst_rate),
                igst_rate: num(item.igst_rate),

                taxable_value: num(gst.taxable),
                cgst_amount: num(gst.cgst),
                sgst_amount: num(gst.sgst),
                igst_amount: num(gst.igst),

                line_total_before_discount: parseFloat(lineBefore.toFixed(2)),
                line_total_after_discount: parseFloat(lineAfter.toFixed(2)),
    ...
    window.buildQuotationItemsForPayload = window.buildInvoiceItemsForPayload;

    document.addEventListener('DOMContentLoaded', function () {
        if (!isManualSellerSale()) return;

        const addBtn = document.getElementById('product-add-button');
        if (addBtn) {
            addBtn.disabled = false;
            addBtn.removeAttribute('disabled');
            addBtn.title = 'Add manual seller sale item';
...
/* =========================================================
   FINAL SELLER MANUAL SALE OVERRIDE
   Seller-entered products are invoice-only.
   No products table lookup. No stock check. No stock deduction.
========================================================= */
(function () {
    function isManualSaleMode() {
        return !!document.getElementById('manual-product-name');
    }
    ...
    function getManualHsnData() {
        ...
        const opt = hsnSelect.options[hsnSelect.selectedIndex];
        const rawHsn = hsnSelect.value || opt?.dataset?.hsn || opt?.getAttribute('data-hsn') || '';
        const isNonGst = String(rawHsn).toUpperCase() === 'NON-GST';

        return {
            hsn: isNonGst ? '' : rawHsn,
            cgst: isNonGst ? 0 : toNumber(opt?.dataset?.cgst || opt?.getAttribute('data-cgst') || 0),
            sgst: isNonGst ? 0 : toNumber(opt?.dataset?.sgst || opt?.getAttribute('data-sgst') || 0),
            igst: isNonGst ? 0 : toNumber(opt?.dataset?.igst || opt?.getAttribute('data-igst') || 0)
        };
    }

    // Manual sale must never search products table.
    window.findProductById = function () { return null; };
    window.findProductByBarcode = function () { return null; };
    window.handleBarcodeScan = function (event) {
        if (event && event.preventDefault) event.preventDefault();
    ...
    // Manual sale does not need products API.
    window.loadProducts = async function () {
        PRODUCTS = [];
        BARCODE_MAP = {};
        console.log('Manual seller sale: products loading skipped.');
        return true;
    };

    window.populateProductDropdownFromSearch = function () {
        const select = document.getElementById('search-product');
        if (select) {
            select.innerHTML = '<option value="">Manual sale - product table not used</option>';
    ...
    window.loadInitialData = async function () {
        try {
            ...
            console.log('Manual seller sale ready. Product stock module skipped.');
        } catch (error) {
            console.warn('Manual seller sale initial data warning:', error);
        }
    };
    ...
    window.addProductToCart = function () {
        try {
            ...
            if (!nameInput) {
                showToast('Manual product name field missing', 'danger');
                return;
            }
    ...
    window.updateCartItemQuantity = function (index, newQty) {
        if (!CART[index]) return;

        newQty = toNumber(newQty);

        if (newQty <= 0) {
            if (typeof removeCartItem === 'function') removeCartItem(index);
            return;
        }
    ...
    window.liveUpdateCartItemQuantity = window.updateCartItemQuantity;
    ...
    window.updateCartItemPrice = function (index) {
        ...
        const discount = toNumber(item.discount_value);

        let finalPrice = price;
        if (item.discount_type === 'percentage' && discount > 0) {
            finalPrice = price * (1 - (discount / 100));
        } else if (item.discount_type === 'fixed' && discount > 0) {
    ...
    window.updateCartItemPriceType = function (index, priceType) {
        if (!CART[index]) return;
        CART[index].price_type = 'manual';
        if (typeof saveCartToSession === 'function') saveCartToSession();
        if (typeof renderCart === 'function') renderCart();
    ...
    window.updateCartPriceTypes = function () {
        CART.forEach(item => {
    ...
    window.buildInvoiceItemsForPayload = function (totals) {
        ...
        return CART.map(function (item, index) {
            ...
            const price = toNumber(item.price);
            const discountShare = toNumber(distribution[index] || item.overall_discount_share || 0);
            const lineBefore = qty * price;
            const lineAfter = Math.max(0, lineBefore - discountShare);
            ...
            return {
                ...
                taxable_value: toNumber(gst.taxable),
                cgst_amount: toNumber(gst.cgst),
                sgst_amount: toNumber(gst.sgst),
                igst_amount: toNumber(gst.igst),
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (unitConvert) {
            unitConvert.disabled = true;
            unitConvert.title = 'Manual sale does not use stock unit conversion';
        }
    });
})();
