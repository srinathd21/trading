...
$metaTitle       = $metaTitle ?? ('Category | ' . $displayName);
$metaDescription = $metaDescription ?? ('Browse category products from ' . $displayName);
...
$currentCategoryId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
if (!function_exists('category_open_build_url')) {
    function category_open_build_url(array $params = []): string {
        global $slug, $currentCategoryId;
        $base = 'storefront.php?slug=' . urlencode((string)$slug) . '&page=category-open&id=' . (int)$currentCategoryId;
        if (!empty($params)) {
...
if (!$currentCategory) {
    echo '<div style="padding:40px;text-align:center;font-family:Arial,sans-serif;">Category not found.</div>';
    return;
}

$currentCategoryName = (string)($currentCategory['category_name'] ?? 'Category');
$currentCategoryDesc = (string)($currentCategory['description'] ?? 'Browse products in this category.');

/* =========================
   FILTER PRODUCTS
========================= */
$filteredProducts = array_filter($products, function ($item) use ($currentCategoryId) {
...
<head>
  ...
  <title><?php echo sf_h($currentCategoryName . ' | ' . $displayName); ?></title>
  <meta name="description" content="<?php echo sf_h($currentCategoryDesc); ?>">
...
<section class="shop-toolbar">
  <div class="container-fluid px-lg-5 px-3">
    <form method="GET" action="storefront.php">
      ...
      <input type="hidden" name="page" value="category-open">
      <input type="hidden" name="id" value="<?php echo (int)$currentCategoryId; ?>">
      ...
      <div class="row g-3 align-items-center">
        ...
        <div class="col-lg-3 col-md-6">
          <a href="<?php echo sf_h($categoryPageUrl); ?>" class="btn btn-outline-dark w-100" style="min-height:44px;display:flex;align-items:center;justify-content:center;">
            All Categories
          </a>
        </div>
...
<section class="section-space pt-0" id="products">
  <div class="container-fluid px-lg-5 px-3">
    <div class="section-title">
      <h2><?php echo sf_h($currentCategoryName . ' Products'); ?></h2>
