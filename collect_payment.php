...
// Optional but recommended: keep connection collation consistent
try {
...
$pending_amount = round((float)($invoice['pending_amount'] ?? 0), 2);

$balance_amount = $pending_amount > 0 ? $pending_amount : max(0, $total_amount - $paid_amount);

if (($invoice['payment_status'] ?? '') === 'paid' || $balance_amount <= 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($payment_amount <= 0 || $payment_amount > $balance_amount) {
        ...
        try {
            ...
            $live_total   = round((float)($liveInvoice['total'] ?? 0), 2);
            $live_paid    = round((float)($liveInvoice['paid_amount'] ?? 0), 2);
            $live_pending = round((float)($liveInvoice['pending_amount'] ?? 0), 2);
            $live_balance = $live_pending > 0 ? $live_pending : max(0, $live_total - $live_paid);
            ...
            // IMPORTANT FIX:
            // convert empty strings to NULL so SQL does not need <> ''
            ...
            $new_paid    = round($live_paid + $payment_amount, 2);
            $new_pending = round(max(0, $live_total - $new_paid), 2);
            ...
            // Record exact payment in invoice_payments
            ...
            // Also store a summary row for customer ledger/reporting
            $summaryStmt = $pdo->prepare("
                INSERT INTO customer_payments
                (business_id, customer_id, invoice_id, total_amount, payment_method, reference_no, payment_date, notes, created_by, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
            ");
            $summaryStmt->execute([
                $business_id,
            ...
            if ($pointsBasis > 0 && $new_paid > 0) {
                $checkPointStmt = $pdo->prepare("
                    ...
                    WHERE business_id = ? AND invoice_id = ? AND transaction_type = 'earned'
                ");
            ...
            <div class="container-fluid">
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label text-muted mb-1">Invoice Number</label>
                                <div class="fw-semibold"><?= htmlspecialchars($invoice['invoice_number']) ?></div>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label text-muted mb-1">Invoice Date</label>
                                <div class="fw-semibold"><?= !empty($invoice['created_at']) ? date('d M Y h:i A', strtotime($invoice['created_at'])) : '—' ?></div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-6">
                                <label class="form-label text-muted mb-1">Customer Name</label>
                                <div class="fw-semibold"><?= htmlspecialchars($invoice['customer_name'] ?? 'Walk-in Customer') ?></div>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label text-muted mb-1">Phone</label>
                                <div class="fw-semibold"><?= htmlspecialchars($invoice['customer_phone'] ?? '—') ?></div>
                            </div>

                            <div class="col-md-12">
                                <label class="form-label text-muted mb-1">Address</label>
                                <div class="fw-semibold"><?= nl2br(htmlspecialchars($invoice['customer_address'] ?? '—')) ?></div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <form method="POST" id="paymentForm" novalidate>
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <small class="text-muted">Maximum payable now: ₹<?= number_format($balance_amount, 2) ?></small>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label" id="reference_label">Reference / Transaction No</label>
                                    <input
                                        type="text"
                                        class="form-control"
                                        name="reference"
                                ...
                                <div class="col-12 d-flex gap-2">
                                    <button type="button" class="btn btn-outline-secondary" id="pay_half_btn">
                                        Pay Half
                                    </button>
                                    <button type="button" class="btn btn-outline-primary" id="pay_full_btn">
                                        Pay Full
                                    </button>
                                    <button type="submit" class="btn btn-success">
                                        <i class="bx bx-check-circle me-1"></i> Save Payment
                                    </button>
                                </div>
                            </div>
                        </form>
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-light d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-primary"><?= count($payments) ?> record(s)</span>
                    </div>
                    <div class="card-body">
                                ...
                                <table class="table table-bordered table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        <?php foreach ($payments as $pay): ?>
                                            <tr>
                                                <td>
                                                    <?= !empty($pay['payment_date']) ? date('d M Y', strtotime($pay['payment_date'])) : '—' ?>
                                                ...
                                                <td><?= htmlspecialchars(ucwords(str_replace('_', ' ', $pay['payment_method'] ?? '—'))) ?></td>
                                                <td><?= htmlspecialchars($pay['reference'] ?: '—') ?></td>
                                                <td><?= htmlspecialchars($pay['notes'] ?: '—') ?></td>
                                                <td><?= !empty($pay['created_by']) ? 'User #' . (int)$pay['created_by'] : '—' ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const payHalfBtn = document.getElementById('pay_half_btn');
    const payFullBtn = document.getElementById('pay_full_btn');
    const balanceAmount = <?= json_encode((float)$balance_amount) ?>;

    function updateReferenceLabel() {
        const method = methodSelect.value;
        if (method === 'upi') {
            refLabel.textContent = 'UPI Reference';
        } else if (method === 'bank') {
            refLabel.textContent = 'Bank Reference';
        } else if (method === 'cheque') {
            refLabel.textContent = 'Cheque Number';
        } else {
            refLabel.textContent = 'Reference / Transaction No';
        }
    }

    if (methodSelect) {
        methodSelect.addEventListener('change', updateReferenceLabel);
        updateReferenceLabel();
    }

    if (payHalfBtn) {
        payHalfBtn.addEventListener('click', function () {
            amountInput.value = (Math.round((balanceAmount / 2) * 100) / 100).toFixed(2);
        });
    }
